...
export const removeNonWordChars = (value: string) => value.replaceAll(/\W/g, ' ');
...
const extractMetaNumber = (metaChar: string, sourceFile: SourceFile) => {
  const result = getRelativePath(sourceFile).match(new RegExp(`/${metaChar}(\\d+)/`));
  if (!result) return;
  const [, value] = result;
  if (value) {
...
export const parseSeasonNumber = (sourceFile: SourceFile) => extractMetaNumber('S', sourceFile);

export const parseEpisodeNumber = (sourceFile: SourceFile) => extractMetaNumber('E', sourceFile);
